...
st.markdown("<p style='text-align: center; color:#FF0000; font-size:40px; font-weight: bold; '>Salesman Account Change<br> Request Form 1-1</br></p>",unsafe_allow_html=True)
...
with col1_2:
    ...
    st.write('Team '+st.session_state['Red Sales Team 1'])
with col1_3:
    st.subheader('Sales Man')
    b = st.write(st.session_state['Red Sales Man 1'])
with col1_4:
    st.subheader('Reason')
    reason = st.radio(label = ' ',options = ['Retiring', 'Moving to Another Account', 'New'], key = '354')
...
if reason !='Retiring':
    ...
    with col6:
        AT_2 = st.selectbox('Sales Team', options = df, key = '2')
        st.session_state['Red Sales Team 2'] = AT_2
        for i in st.session_state['Red Sales Team 2']:
            new2 = ''.join(map(str,str(i)))
        st.session_state['Red Sales Team 2'] = new2
    with col7:
        con = engine.connect()
        query3  = "select distinct(salesperson_text) from [[dbo]].[TEMPORARY]]] where salesdiv = 'div2' and salesteam_text = ?"
        df3 = con.execute(query3, AT_2)
        AS_2 = st.selectbox('Sales Man', options = df3, key = '3')
        st.session_state['Red Sales Man 2'] = AS_2
    ...
    with colA2:
        ...
        df['From'] = str(AS_2)
        df['To'] = str(st.session_state['Red Sales Man 1'])
        df['Start_From'] = d1
        df['End_date'] = d2
...
# %%
if reason =='Retiring':
    d_f = st.date_input('Please Select Your Working Date: ')
    final = pd.DataFrame({'From':[st.session_state['Red Sales Man 1']], 'TO':['None'],'Reason':['Retiring'],'Last Date':[d_f]})
    with st.form('form 3'):
