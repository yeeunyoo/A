...
st.markdown("<p style='text-align: center; color:#FF0000; font-size:40px; font-weight: bold; '>Salesman Account Change<br> Request Form 2-1</br></p>",unsafe_allow_html=True)
...
with col1_2:
    ...
    st.write('Team '+st.session_state['Red Sales Team 2'])
with col1_3:
    st.subheader('Sales Man')
    b = st.write(st.session_state['Red Sales Man 2'])
with col1_4:
    st.subheader('Reason')
...
if reason !='Retiring':
    ...
    with col6:
        ...
        st.session_state['Red Sales Team 3'] = AT_4
        for i in st.session_state['Red Sales Team 3']:
            new3 = ''.join(map(str,str(i)))
        st.session_state['Red Sales Team 3'] = new3
    with col7:
        con = engine.connect()
        ...
        st.session_state['Red Sales Man 3'] = AS_4
    with col8:
        st.write(' ')
    # %%
    ...
    df['From'] = str(st.session_state['Red Sales Man 2'])
    df['To'] = str(st.session_state['Red Sales Man 3'])
    df['Start_From'] = d1
    df['End_date'] = d2
...
if reason == 'Retiring':
    ...
    final = pd.DataFrame({'From':[st.session_state['Red Sales Man 2']],'TO':['None'], 'Reason':['Retiring'], 'Last Date':[d_f]})
    with st.form('form 3'):
