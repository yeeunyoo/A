...
st.markdown("<p style='text-align: center; color:#1428A0; font-size:40px; font-weight: bold; '>Salesman Account Change<br> Request Form 2</br></p>",unsafe_allow_html=True)
...
with col1_2:
    ...
    st.write('Team '+st.session_state['IVY Sales Team 1'])
with col1_3:
    st.subheader('Sales Man')
    b= st.write(st.session_state['IVY Sales Man 1'])
with col1_4:
    st.subheader('Reason')
...
# %%
    ...
    with col6:
        ...
        st.session_state['IVY Sales Team 2'] = AT_2
        for i in st.session_state['IVY Sales Team 2']:
            new2 = ''.join(map(str,str(i)))
    st.session_state['IVY Sales Team 2'] = new2
    with col7:
        con = engine.connect()
        ...
        st.session_state['IVY Sales Man 2'] = AS_2
    with col8:
        st.write(' ')
...
if reason =='Retiring':
    ...
    final = pd.DataFrame({'From':[st.session_state['IVY Sales Man 1']], 'TO':['None'],'Reason':['Retiring'],'Last Date':[d_f]})
    with st.form('form 3'):
