...
st.markdown("<p style='text-align: center; color:#1428A0; font-size:40px; font-weight: bold; '>Salesman Account Change<br> Request Form 5 </br></p>",unsafe_allow_html=True)
...
with col1_2:
    ...
    st.write('Team '+st.session_state['IVY Sales Team 4'])
with col1_3:
    st.subheader('Sales Man')
    b = st.write(st.session_state['IVY Sales Man 4'])
with col1_4:
    st.subheader('Reason')
...
if reason != 'Retiring':
    ...
    with col6:
        ...
        st.session_state['IVY Sales Team 5'] = AT_6
        for i in st.session_state['IVY Sales Team 5']:
            new4 = ''.join(map(str,str(i)))
        st.session_state['IVY Sales Team 5'] = new4
    with col7:
        con = engine.connect()
        ...
        st.session_state['IVY Sales Man 5'] = AS_6
    ...
    with colA2:
        ...
        df['From'] = str(st.session_state['IVY Sales Man 4'])
...
if reason=='Retiring':
    ...
    final = pd.DataFrame({'From':[st.session_state['IVY Sales Man 4']],'TO':['None'], 'Reason':['Retiring'], 'Last Date':[d_f]})
    with st.form('form 3'):
