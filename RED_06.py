...
st.markdown("<p style='text-align: center; color:#FF0000; font-size:40px; font-weight: bold; '>Salesman Account Change<br> Request Form 3-2</br></p>",unsafe_allow_html=True)
...
with col1_2:
    ...
    st.write('Team '+st.session_state['Red Sales Team 5'])
with col1_3:
    st.subheader('Sales Man')
    b = st.write(st.session_state['Red Sales Man 5'])
with col1_4:
    st.subheader('Reason')
...
if reason !='Retiring':
    ...
    with col6:
        ...
        st.session_state['Red Sales Team 6'] = AT_7
        for i in st.session_state['Red Sales Team 6']:
            new6 = ''.join(map(str, str(i)))
        st.session_state['Red Sales Team 6'] = new6
    with col7:
        con = engine.connect()
        ...
        st.session_state['Red Sales Man 6'] = AS_7
    with col8:
        st.write(' ')
    # %%
    ...
    with colA2:
        ...
        df['From'] = str(st.session_state['Red Sales Man 6'])
...
if reason =='Retiring':
    ...
    final = pd.DataFrame({'From':[st.session_state['Red Sales Man 5']],'TO':['None'], 'Reason':['Retiring'], 'Last Date':[d_f]})
    with st.form('form 3'):
