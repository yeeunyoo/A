...
st.markdown("<p style='text-align: center; color:#FF0000; font-size:40px; font-weight: bold; '>Salesman Account Change<br> Request Form 3-1</br></p>",unsafe_allow_html=True)
...
with col1_2:
    ...
    st.write('Team '+st.session_state['Red Sales Team 4'])
with col1_3:
    st.subheader('Sales Man')
    b = st.write(st.session_state['Red Sales Man 4'])
with col1_4:
    st.subheader('Reason')
    reason = st.radio(label = ' ', options = ['Retiring', 'Moving to Another Account', 'New'], key = '36')
...
if reason!='Retiring':
    ...
    with col6:
        AT_6 = st.selectbox('Sales Team', options = df, key = '2')
        st.session_state['Red Sales Team 5'] = AT_6
        for i in st.session_state['Red Sales Team 5']:
            new4 = ''.join(map(str, str(i)))
        st.session_state['Red Sales Team 5'] = new4
    with col7:
        con = engine.connect()
        query3 = "select distinct(salesperson_text) from [[dbo]].[TEMPORARY]]] where salesdiv = 'div2' and salesteam_text = ?"
        df4 = con.execute(query3, AT_6)
        AS_6 = st.selectbox('Sales Man', options=df4, key = '3')
        st.session_state['Red Sales Man 5'] = AS_6
    ...
    df['From'] = str(st.session_state['Red Sales Man 4'])
    df['To'] = str(st.session_state['Red Sales Man 5'])
    df['Start_From'] = d1
    df['End_date'] = d2
...
if reason=='Retiring':
    ...
    final = pd.DataFrame({'From':[st.session_state['Red Sales Man 4']],'TO':['None'], 'Reason':['Retiring'], 'Last Date':[d_f]})
    with st.form('form 3'):
